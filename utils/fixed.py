...
MATCH_DETAILS_PATH = os.path.join(MAIN_PATH, DATA_PATH, 'match_details.csv')
MATCH_DATA = pd.read_csv(MATCH_DATA_PATH, encoding='utf-8', sep=';', quotechar='"', index_col=0, skip_blank_lines=True,
                         converters={'hashtags_team1': literal_eval, 'hashtags_team2': literal_eval})
MATCH_DATA = MATCH_DATA.dropna(0, how='all')
MATCH_DATA.index = MATCH_DATA.index.map(int)
...
def create_match_df(match):
    ...
    tweets_df = pd.read_csv(file_path, sep=';', encoding='utf-8', lineterminator='\n', index_col=0,
                            parse_dates=['tweetCreated'])
    try:
        tweets_df['team\r'] = tweets_df['team\r'].str.replace('\r', '')
        tweets_df = tweets_df.rename(columns={'team\r': 'team'})
    except:
        pass
    tweets_df = tweets_df[
        ((tweets_df['tweetCreated'] >= match_start) & (tweets_df['tweetCreated'] <= first_part_end)) | (
                (tweets_df['tweetCreated'] >= second_part_start) & (
                tweets_df['tweetCreated'] <= match_end))].reset_index(drop=True)
    return tweets_df
...
def processTweet(tweet, translate_slang=True, SLANG_DICT=load_slang(SLANG_PATH)):
    CONTRACTION_DICT = {"ain't": "is not", "aren't": "are not", "can't": "cannot",
                        ...
                        "needn't": "need not", "needn't've": "need not have", "o'clock": "of the clock", "one's": "",
                        "someone's": "",
                        "oughtn't": "ought not", "oughtn't've": "ought not have", "shan't": "shall not",
                        "sha'n't": "shall not", "shan't've": "shall not have", "she'd": "she would",
                        "she'd've": "she would have", "she'll": "she will", "she'll've": "she will have",
                        "she's": "she is", "shoulda": "should have", "should've": "should have",
...
def tokenizeTweet(tweet, remove_stopwords=True, remove_shortwords=True, token_len=2):
    ...
    NEUTRAL_WORDS = ['v', 'vs', 'league', 'pl', 'link', 'live', 'stream', 'hd', 'matchday', 'epl', 'game', 'iphone',
                     'android', 'pc', 'mobile', 'ipad']
    ADDITIONAL_STOP_WORDS = ['amp', 'rt', 'via', 'fav', "'s"]
    ALL_STOP_WORDS = STOP_WORDS + NEUTRAL_WORDS + ADDITIONAL_STOP_WORDS
